...
dat = dat %>% left_join(dat %>%  
  dplyr::select(pragmaid, period, interv.type, interv_id) %>%
  drop_na() %>%
...
dat_qwt = get_treatment_series_at_resolution(dat,
  ...
  left_join(dat_cov %>% dplyr::select(pragmaid, all_of(covariates)), by =c("pragmaid" = "pragmaid"))
...
dat_qwt_num %>% 
  dplyr::select(pragmaid, predclass) %>%
...
dat_qwt_num %>%
  dplyr::select(pragmaid, predclass, predclassC) %>%
  write_csv(here("output",
                 paste0("1_sequence_data_24m_monthly_lca_results_",Sys.Date(),".csv")))
...
frac = data.frame(pragmaid = dat$pragmaid, predclass = dat$predclass %>% as.factor()) %>%
  ...
  names(res) = frac %>% dplyr::select(predclass) %>% pull()
  return(res) 
}
...
dat_pred = dat_qwt_num %>% dplyr::select(pragmaid, predclass) %>% right_join(dat_qwt)
...
dat_pred %>% 
  ...
  ungroup() %>% 
  mutate(frac = n/n_ids) %>% 
  dplyr::select(var,n_treats, predclass, frac) %>% 
